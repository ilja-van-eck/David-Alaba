...
function initPageLoad(data){
  ...
  let tl = gsap.timeline({
    ...
    if(fadeTargets){tl.from(fadeTargets,{autoAlpha:0,yPercent:50,duration:1},0.5)}
}
...
function initGalleryFilters(next) {
  ...
  let notice = next.querySelector(".notice")
  let wrap = next.querySelector("[data-drag-container]")
  ...
  gsap.fromTo(notice,{autoAlpha: 0.5},{
    ease:"linear",
    autoAlpha:0,
    scrollTrigger:{
      trigger: wrap,
...
const createNoiseEffect = (next) => {
    let canvas, ctx, wWidth, wHeight, animationFrameId;

    const initCanvas = () => {
        canvas = next.querySelector('.noise-canvas');
        if (!canvas) return;
  ...
  transitions: [
    {
      ...
      leave(data) {
        ...
        if (data.current.namespace === 'home' && data.current.container.noiseEffect) {
          data.current.container.noiseEffect.destroy();
        }
  ...
  views: [
    {
      ...
      afterEnter(data) {
        ...
        initHome(next);
          const noiseEffect = createNoiseEffect(next);
          noiseEffect.init();
          data.next.container.noiseEffect = noiseEffect;
      },
    },
    {
      namespace:"journey",
